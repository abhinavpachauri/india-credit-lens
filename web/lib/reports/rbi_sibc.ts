...
import rawL1 from "../../public/data/sibc_l1_annotations.json";
...
// ── L1 computed annotations (sourced from signal evaluation JSON) ─────────────
// Generated by: python3 analysis/generate_analysis_report.py
// L2/L3 annotations remain authored in the ANNOTATIONS object below.

const L1_SECTIONS = (rawL1 as any).sections as Record<string, SectionAnnotations>;

function mergeAnnotations(sectionId: string): SectionAnnotations {
  const authored = ANNOTATIONS[sectionId] ?? { insights: [], gaps: [], opportunities: [] };
  const computed = L1_SECTIONS[sectionId]  ?? { insights: [], gaps: [], opportunities: [] };

  // L2/L3 from authored (filter out any stale layer:1 entries during migration)
  const authoredInsights     = authored.insights.filter((a) => (a.layer ?? 2) !== 1);
  const authoredGaps         = authored.gaps.filter((a) => (a.layer ?? 2) !== 1);
  const authoredOpportunities = authored.opportunities.filter((a) => (a.layer ?? 3) !== 1);

  return {
    // L1 computed first (data signals), then L2/L3 authored context
    insights:     [...computed.insights,     ...authoredInsights],
    gaps:         [...computed.gaps,         ...authoredGaps],
    opportunities: [...computed.opportunities, ...authoredOpportunities],
  };
}
