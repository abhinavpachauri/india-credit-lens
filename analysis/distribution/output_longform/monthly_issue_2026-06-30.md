# India Credit Lens — credit and payments

Both halves are on the same data month: May 2026 credit data and May 2026 payments data.

Total bank credit outstanding reached ₹215.2L Cr in May 2026, the highest level on record. Volume climbed ₹3.0L Cr period-on-period (₹212.1L Cr → ₹215.2L Cr), reversing April's ₹1.5L Cr decline and resuming the upward trajectory from ₹178.7L Cr in January 2025.


## Credit — May 2026


- **₹215.2L Cr** — Bank credit (17.7% YoY)
- **15.4% YoY** — Personal loans
- **17.5% YoY** — Industry


## How each sector is growing


| Sector | YoY growth | Trend |
| --- | --- | --- |
| **Bank Credit** |  |  |
| Food Credit | 94.1% | ↗ accelerating |
| Bank Credit | 17.7% | ↗ accelerating |
| Non-food Credit | 17.4% | ↗ accelerating |
| **Agriculture** |  |  |
| Agriculture | 14.9% | ↗ accelerating |
| **Industry** |  |  |
| Micro and Small | 26.2% | ↘ decelerating |
| Medium | 21.2% | ↗ accelerating |
| Industry | 17.5% | ↗ accelerating |
| Large | 14.4% | ↗ accelerating |
| **Services** |  |  |
| Services | 20.4% | ↗ accelerating |
| **Personal Loans** |  |  |
| Gold Loans | 105.5% | ↘ decelerating |
| Vehicle Loans | 17.3% | ↘ decelerating |
| Personal Loans | 15.4% | ↘ decelerating |
| Education Loans | 14.0% | ↗ accelerating |
| Other Personal Loans | 12.5% | ↘ decelerating |
| Housing | 10.9% | ↘ decelerating |
| Advances vs Shares/Bonds | 8.3% | ↗ accelerating · turned |
| Advances vs Fixed Deposits | 8.1% | ↘ decelerating |
| Credit Card Outstanding | 1.3% | ↘ decelerating |
| Consumer Durables | -2.6% | ↗ accelerating |
| **Priority Sector** |  |  |
| Social Infrastructure | 70.7% | ↗ accelerating |
| Housing | 39.0% | ↘ decelerating |
| Micro and Small Enterprises | 23.7% | ↘ decelerating |
| Education Loans | 22.6% | ↘ decelerating |
| Medium Enterprises | 17.8% | ↗ accelerating |
| Renewable Energy | 17.5% | ↗ accelerating |
| Agriculture | 13.8% | ↘ decelerating |
| Weaker Sections | 9.1% | ↘ decelerating |
| Export Credit | -9.7% | ↗ accelerating |
| Others | -11.2% | ↗ accelerating |

*Trend shows momentum in the growth rate: ↗ accelerating (rate rose vs last month), ↘ decelerating (rate fell), → steady. 'turned' marks the rarer event — a sector that flipped between growing and shrinking.*


## Where the mix is shifting

Within services lending, the mix is tilting toward Non-Banking Financial Companies (NBFCs). Its slice of services credit grew by 3.4pp over the year, while Other Services gave up 2.1pp. Put simply — a bigger share of every rupee lent to services now goes to Non-Banking Financial Companies (NBFCs).


## The reads that matter


**Bank credit at ₹215.2L Cr — new all-time high**


Total bank credit outstanding reached ₹215.2L Cr in May 2026, the highest level on record. Volume climbed ₹3.0L Cr period-on-period (₹212.1L Cr → ₹215.2L Cr), reversing April's ₹1.5L Cr decline and resuming the upward trajectory from ₹178.7L Cr in January 2025.


> 📊 **[CHART — replace with screenshot]** indiacreditlens.com → Bank Credit → Absolute view → highlight: Bank Credit


**Credit card outstanding fell to ₹2.95L Cr — second straight decline**


Credit card outstanding stood at ₹2.95L Cr in May 2026, down ₹3,481 crore from Apr 2026. Outstanding declined for the second consecutive period: ₹2.98L Cr in Apr 2026 → ₹2.95L Cr in May 2026.


> 📊 **[CHART — replace with screenshot]** indiacreditlens.com → Personal Loans → Absolute view → highlight: Credit Card Outstanding


## Payments — May 2026


- **12.0 crore** — Credit cards in force (8.5% YoY)
- **102.8 crore** — Debit cards in force (2.8% YoY)
- **1.2 crore** — POS terminals (-0.5% YoY)


## Fleet vs usage

A few things that usually move together have come apart this year:

The number of debit cards and spending on them have pulled apart this year. The number of debit cards moved by +2.8% while spending on them moved by -4.3% — a gap of 7.1pp, which is the space between having a card and using it.

The POS machines deployed and the money flowing through them have pulled apart this year. The POS machines deployed moved by -0.5% while the money flowing through them moved by +4.2% — a gap of -4.7pp, which is the space between how many machines there are and how much they handle.

The ATMs deployed and the cash withdrawn from them have pulled apart this year. The ATMs deployed moved by -2.7% while the cash withdrawn from them moved by -6.1% — a gap of 3.3pp, which is the space between the size of the ATM fleet and how much it dispenses.


## The biggest banks this month


| Bank | Credit cards in force |
| --- | --- |
| HDFC BANK LTD | 2.7 crore |
| STATE BANK OF INDIA | 2.2 crore |
| ICICI BANK LTD | 1.9 crore |
| AXIS BANK LTD | 1.6 crore |
| KOTAK MAHINDRA BANK LTD | 47.5 lakh |


| Bank | Debit cards in force |
| --- | --- |
| STATE BANK OF INDIA | 25.5 crore |
| BANK OF BARODA | 8.5 crore |
| HDFC BANK LTD | 6.3 crore |
| CANARA BANK | 6.0 crore |
| UNION BANK OF INDIA | 5.7 crore |


| Bank | POS terminals |
| --- | --- |
| RBL BANK LTD | 27.6 lakh |
| AXIS BANK LTD | 25.5 lakh |
| HDFC BANK LTD | 18.7 lakh |
| STATE BANK OF INDIA | 17.9 lakh |
| ICICI BANK LTD | 17.3 lakh |


## The reads that matter


**ATM volume share 80.61% — holding near median**


ATM withdrawals represented 80.61% of total debit card transaction volume in May 2026. Share edged down 80.84% → 80.61%, slipping 0.23pp but remaining near the median (80.1%) of the 29-month range.


> 📊 **[CHART — replace with screenshot]** indiacreditlens.com/payments → Debit Cards → Absolute view → highlight: Total


**67.7 UPI QR codes per POS terminal — ratio at record high**


There are now 67.7 UPI QR codes for every POS terminal in May 2026, up 0.32 from 67.3 in Apr 2026. Ratio rose 67.3× → 67.7×, the highest level on record — the gap has widened by 0.3 points as UPI QR scaled 90 lakh codes while POS terminals added 78,278 units.


> 📊 **[CHART — replace with screenshot]** indiacreditlens.com/payments → Infrastructure → Absolute view → highlight: Total

---

The interactive charts behind every number are on the dashboards: indiacreditlens.com for credit, indiacreditlens.com/payments for payments.

*How this is made: the data goes through an automated pipeline with validation gates at every step. Every number is machine-checked against the RBI source file; the two reads in each half are chosen by an editor from a shortlist the pipeline ranks by how much each moved.*
