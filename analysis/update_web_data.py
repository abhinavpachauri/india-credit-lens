...
Must be run after any new SIBC xlsx file is added to analysis/rbi_sibc/{period}/raw/.

What it does:
  1. Calls analysis/rbi_sibc/lib/consolidate.py on all SIBC*.xlsx files in period raw/ dirs
  2. Writes directly to web/public/data/rbi_sibc_consolidated.csv
...
def main(dry_run: bool = False):
    ...
    if not xlsx_files:
        print(f"ERROR: No SIBC*.xlsx files found in analysis/rbi_sibc/*/raw/", file=sys.stderr)
        sys.exit(1)

    print(f"\n  Found {len(xlsx_files)} xlsx file(s):")
    for f in xlsx_files:
        print(f"    {f.relative_to(ANALYSIS)}")

    if dry_run:
        print("\n  [dry-run] Would consolidate and write to web/public/data/")
        print(f"  Source : analysis/rbi_sibc/*/raw/")
    ...
    try:
        ...
        print(f"ERROR: Cannot import consolidate.py from {RBI_SIBC_LIB}: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    # Write directly to web/public/data/
    WEB_DATA.mkdir(parents=True, exist_ok=True)
    df.to_csv(WEB_CSV, index=False)
    print(f"  Wrote   : {WEB_CSV}  ({len(df)} rows)")
