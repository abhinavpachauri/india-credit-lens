...
| `analysis/signals/registry.json` | Universal signal catalog — 253 signals, layer 1/2/3 tagged; all Layer 1 signals have compute specs (SIBC + ATM/POS). Counts here are checked by `reconcile.py` |
...
correctness, not *technical*. ~~Gaps: authoritative `ARCHITECTURE.md`~~ **(DONE — `ARCHITECTURE.md` +
the generated half, freshness-guarded)**; ~~**unit tests for the deterministic core** (1 file/9 tests)~~
**(DONE — the suite is now the gate's first stage; see `reconcile.py`'s count check for the live figure)**; the **`analysis/` restructure** (ingestion/gate layer is copy-per-pipeline → `core/`
...
scripts (`source_claims`, mermaid gen, `validate`, `build_behavioral_layer`,
