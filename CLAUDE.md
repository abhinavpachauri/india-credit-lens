...
| signal compute layer | **Live** — `analysis/signals/` — registry.json (**286 signals**), signals.db (SQLite — sole store); compute engine in signals/compute/; Check 2e validates DB + registry, Check 2f/5b2 verifies DB == fresh recompute from CSV |
...
| `analysis/signals/registry.json` | Universal signal catalog — 286 signals, layer 1/2/3 tagged; all Layer 1 signals have compute specs (SIBC + ATM/POS). Counts here are checked by `reconcile.py` |
