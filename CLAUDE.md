...
| signal compute layer | **Live** — `analysis/signals/` — registry.json (**527 signals**), signals.db (SQLite — sole store); compute engine in signals/compute/; Check 2e validates DB + registry, Check 2f/5b2 verifies DB == fresh recompute from CSV |
