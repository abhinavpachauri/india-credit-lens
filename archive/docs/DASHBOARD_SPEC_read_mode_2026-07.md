# Dashboard read mode — design record (July–August 2026)

> **Archived 2026-09-24 from `analysis/DASHBOARD_SPEC.md` §1–§14 (+ §12.1).** This is the design of
> the July read mode: three planes, the Brief/Full/Deep depth ladder, the grid front door, the chip
> strip and the rail. **§17–§20 of the live spec replaced its navigation and layout** (one table per
> dimension, a rail that collapses into a chart column), and the Deep view is disabled. It is kept
> because code comments and the live spec still cite §11 and §14, and because the reasoning in
> §1–§3 (why 131 flat cards failed) still explains decisions the live spec builds on.
> Section numbers are unchanged so those citations still resolve.
